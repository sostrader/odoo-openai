...
class OpenAiFineTuning(models.Model):
    ...
    name = fields.Char()
    training_model = fields.Selection(
        selection=_get_training_model_list, default="gpt-3.5-turbo"
    )
    ...
    question_answer_ids = fields.Many2many(
    ...
    def _compute_question_answers(self):
        for rec in self:
            domain = (
                safe_eval(rec.question_answer_domain, SAFE_EVAL_BASE, {"self": rec})
                if rec.question_answer_domain
                else []
            )
    ...
    def create_fine_tuning(self):
        ...
        res = client.fine_tuning.jobs.create(
            training_file=self.training_file_id, model=self.training_model
        )
